...
- 先阅读本《设计规范索引》，再根据验证范围加载对应 references：
  - **色彩验证**：必读 `references/color-tokens.md`（ACCESS 主题色板、图表色板、语义映射、默认 7 号色、主文本 Gray-9、普通按钮 Gray-1 等）。
...
| 验证项 | 规范要求 | 检查方式 |
|--------|----------|----------|
| 色值是否在规范色板内 | 所有填充/描边/文字颜色须为基础 UI 色板（Brand/Blue/Green/Red/Yellow/Gray/Translucent）或图表专用色板（Chart Palette / Comparison Pairs）中定义的色值 | 使用 MCP 时：对比 get_variable_defs / get_metadata 返回的色值与色彩系统 md；否则对比用户提供的 Hex |
| 图表系列颜色 | 多系列图表须使用 `color-chart-series-1` ~ `color-chart-series-9`，并按指标数量顺序取前 N 个颜色；双系列对比图优先使用文档规定的对比色组合 | 检查图表系列颜色是否按顺序使用，是否出现跳色、随机取色或错误复用 |
...
| 模块 | 文件路径 | 内容摘要 | 典型适用场景 |
|------|----------|----------|--------------|
| color-tokens | `references/color-tokens.md` | 品牌/辅色/中性/状态色、图表色板、背景与文本规则 | 配色、Figma 色彩校验、按钮/标签/状态色、图表配色 |
...
| 卡片列表、有/无侧导卡片、独立筛选区卡片 | `layout-grid`、`components` |
| 数据看板、运营看板、监控大屏、数据概览页（有/无侧导） | `layout-grid`、`components`、`color-tokens`、`spacing-scale` |
| 布局选型、顶部导航、侧导、白卡分块、典型布局 | `layout-grid` |
| 浮层容器、抽屉、弹窗、Modal、Dialog、Popover | `layout-grid`、`components`、`spacing-scale` |
| 标题旁问号说明 icon、标题释义浮层 | `layout-grid`、`components`、`icons` |
...
- **独立筛选区卡片列表**：  
  - 必读：`references/layout-grid.md`（独立筛选区章节）、`references/components.md`（RadioButton/Checkbox Simple 按钮组）、`references/spacing-scale.md`。

- **数据看板/运营看板/监控大屏（有/无侧导）**：  
  - 必读：`references/layout-grid.md`、`references/components.md`、`references/color-tokens.md`、`references/spacing-scale.md`；涉及图表组件时，优先参考 `references/components.md` 中的 `Chart` 小节与 `references/color-tokens.md` 中的图表色板。
