...
- **产出要求**：
  - 在生成设计稿描述（或 Figma 结构描述）时，需明确标注关键规范的引用，例如：
    - 「颜色使用：主按钮采用 `主品牌色 / Primary`，详见《色彩系统》」
    - 「正文采用 `Body / 14px / 行高 22px`，详见《字体与字号系统》」
...
- **若已安装并启用 figma-desktop MCP**：必须优先通过 MCP 获取设计文件数据，无需用户再提供截图或色值列表。
  ...
  - 使用 **`get_variable_defs`**：获取文件中使用的变量定义（颜色、字体、间距等），用于与 `references/色彩系统.md` 中的色板对照。
...
- 先阅读本《设计规范索引》，再根据验证范围加载对应 references：
  - **色彩验证**：必读 `references/色彩系统.md`（ACCESS 主题色板、语义映射、默认 7 号色、主文本 Gray-9、普通按钮 Gray-1 等）。
- 将用户提供的色值/截图描述与规范逐条对照，不做主观猜测；若材料不足以判断某条规范，在报告中标注「无法验证（需补充材料）」。

### 3. 色彩合规验证项（当前必检）

按 `references/色彩系统.md` 执行，至少检查：
...
| 模块 | 文件路径 | 内容摘要 | 典型适用场景 |
|------|----------|----------|--------------|
| 色彩系统 | `references/色彩系统.md` | 品牌/辅色/中性/状态色、背景与文本规则 | 配色、Figma 色彩校验、按钮/标签/状态色 |
| 字体与字号系统 | `references/字体与字号系统.md` | 字体族、字号层级、行高、字重、中英混排 | 标题/正文层级、排版、字号选择 |
| 布局与栅格系统 | `references/布局与栅格系统.md` | 最低适配宽度、顶部导航、侧导、白卡分块、典型布局（表单/表格/卡片） | 页面结构、布局选型、带/无侧导、表单/表格/卡片列表 |
| 间距与尺寸系统 | `references/间距与尺寸系统.md` | Height/水平/垂直间距/圆角 token、使用建议、表单间距索引 | 组件尺寸、页面边距、卡片间距、通用 token |
| 组件库规范 | `references/组件库规范.md` | OneUI 组件清单、各组件 best practices、子类型（如 Checkbox/Radio/Form） | 选组件、写表单/表格/按钮/弹窗等 |
| 图标使用规范 | `references/图标使用规范.md` | dls-icons-react、尺寸规范、线/面性、Icon 使用建议 | 用图标、图标尺寸与风格 |
| 表单页面规范 | `references/表单页面规范.md` | 步骤条表单结构、白卡/圆角/边距、表单间距校验规则、字段与按钮 | 多步骤表单、表单页布局、表单间距与校验 |
| 输入模板 | `references/输入模板.md` | 设计需求输入结构 | 将需求结构化、填写设计输入 |

## 索引场景速查

| 场景/关键词 | 应加载的 reference |
|-------------|--------------------|
| Figma 设计稿校验、色彩合规 | `色彩系统.md` |
| 整页/整流程界面设计 | `色彩系统`、`字体与字号系统`、`布局与栅格系统`、`间距与尺寸系统`、`组件库规范` |
| 表单页、步骤条表单、多步骤表单、表单布局 | `表单页面规范`、`布局与栅格系统`、`间距与尺寸系统`、`组件库规范` |
| 表单间距、Label 与输入间距、表单项间距、白卡内边距 | `表单页面规范`（含表单间距校验规则）、`间距与尺寸系统`（token 与索引） |
| 表格页、列表页、数据管理、有/无侧导表格 | `布局与栅格系统`、`组件库规范`、`间距与尺寸系统` |
| 卡片列表、有/无侧导卡片 | `布局与栅格系统`、`组件库规范` |
| 布局选型、顶部导航、侧导、白卡分块、典型布局 | `布局与栅格系统` |
| 单个组件（按钮/输入/选择/卡片/弹窗等） | `组件库规范`、`色彩系统`、`字体与字号系统`、`间距与尺寸系统` |
| 图标、图标库、图标尺寸、线型/面型 | `图标使用规范`、`色彩系统` |
| 间距 token、圆角、组件高度、页面边距 | `间距与尺寸系统`；表单内间距细节见 `表单页面规范` |
| 设计需求输入、需求结构化 | `输入模板.md` |
...
- **设计稿规范验证（用户提供 Figma 链接，当前仅色彩）**：  
  - 必读：`references/色彩系统.md`；按「设计稿规范验证流程」执行并输出通过项/不符合项。

- **整页/整流程设计**：  
  - 必读：`references/色彩系统.md`、`references/字体与字号系统.md`、`references/布局与栅格系统.md`、`references/间距与尺寸系统.md`、`references/组件库规范.md`。

- **表单页/步骤条表单/多步骤表单**：  
  - 必读：`references/表单页面规范.md`、`references/布局与栅格系统.md`、`references/间距与尺寸系统.md`、`references/组件库规范.md`；涉及配色与字体时加 `references/色彩系统.md`、`references/字体与字号系统.md`。

- **表格页或卡片列表页（有/无侧导）**：  
  - 必读：`references/布局与栅格系统.md`、`references/组件库规范.md`、`references/间距与尺寸系统.md`。

- **单个组件设计（如按钮、输入、选择、卡片、弹窗）**：  
  - 必读：`references/组件库规范.md`、`references/色彩系统.md`、`references/字体与字号系统.md`、`references/间距与尺寸系统.md`。

- **图标/插画类输出**：  
  - 必读：`references/图标使用规范.md`、`references/色彩系统.md`。

- **状态/交互优化、无障碍**：  
  - 必读：`references/组件库规范.md`（含交互与状态相关）、涉及对比度与触控时查阅项目内无障碍说明；当前 references 无独立「交互与状态」「无障碍规范」文件时，以组件库与色彩/对比度为准。

- **设计需求结构化、输入模板**：  
  - 必读：`references/输入模板.md`。
